...
mod set_pool {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("paypeer", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let ngn_mint: Pubkey = Pubkey::new_unique();
		let usd_mint: Pubkey = Pubkey::new_unique();

		let (swap_authority_pubkey, _) = Pubkey::find_program_address(
			&[b"swapauthority"],
			&program_id,
		);
		...
		let accounts = vec![
			...
			AccountMeta::new(swap_authority_pubkey, false),
			AccountMeta::new(pool_pubkey, false),
			AccountMeta::new_readonly(system_program::id(), false),
		];
		let data = PaypeerInstruction::SetPool(crate::generated::instructions::SetPoolArgs{
			ngn_mint,
			usd_mint,
		});
		let signers = vec![
